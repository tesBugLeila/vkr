...
НОВАЯ СИСТЕМА (БД):
Андрей → Настройки → PUT /api/users/notification-radius
                     ↓
              UPDATE users SET notificationRadius = 20000

Влад создает пост → notifyNeighbors()
                    ↓
            Проверяет КАЖДОГО пользователя:
            - Андрей: 30км > 20км (ЕГО радиус) → НЕ отправлять
            - Мария: 3км <= 50км (ЕЁ радиус) → Отправить




            в проекте не хватает пва, пуш, и смылс в футоре какие фишки данного сайта 
        нужно смс авторизацию в тестовом режиме прикрутить

             нужно переделать что не авторизованный пользователь не может просомтреть контакты!!!!

             ежедневное резервное копирование бд в ночное время


. После разрешения система получает координаты и сохраняет их в профиле пользователя (lastLat, lastLon) для получения гиперлокальных уведомлений. т.е пользователь должен после аворизации высвечиваться запрос на получение данных геолокации в браузере

информация когда можно подьехать и когда готов продукт


рейтинг и отзыв

иконка для предпринимателей и вообще роль продавца (нкжно сделать)
